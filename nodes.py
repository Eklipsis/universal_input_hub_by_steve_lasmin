...
class Universal_Input_Hub_by_Steve_Lasmin:
    ...
    RETURN_TYPES = ("INT", "INT", "STRING", "INT", "NOISE", "INT", "FLOAT", "STRING", "SAMPLER", "STRING", "SIGMAS", "SIGMAS", "MODEL", "STRING", "STRING", "STRING", "CONDITIONING", "CONDITIONING", "STRING")
    RETURN_NAMES = ("width", "height", "preview", "seed", "noise", "steps", "cfg", "sampler_name", "sampler", "scheduler_name", "sigmas", "flux_sigmas", "model", "prompt", "prompt_with_triggers", "negative_prompt", "positive", "negative", "help")
    FUNCTION = "calculate"
    CATEGORY = "utils"
    OUTPUT_NODE = True
    ...
    def calculate(self, max_size, aspect_ratio, width_ratio, height_ratio, multiplier, seed, steps, cfg, sampler, scheduler, model, clip, prompt, trigger_words, negative_prompt):
        ...
        if trigger_stripped:
            prompt_with_triggers = trigger_stripped + ". " + prompt
        else:
